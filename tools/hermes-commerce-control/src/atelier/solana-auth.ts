...
function rawPublicKey(publicKey: KeyObject): Buffer {
  ...
  if (jwk.kty !== "OKP" || jwk.crv !== "Ed25519" || typeof jwk.x !== "string") throw new Error("expected Ed25519 public key");
...
function requireSolanaAddress(address: string): string {
  ...
  if (base58Decode(normalized).length !== SOLANA_PUBLIC_KEY_BYTES) throw new Error("invalid Solana wallet address");
  return normalized;
}

export function privateKeyPkcs8FromSeed(seed: Uint8Array): Buffer {
  const normalized = Buffer.from(seed);
  if (normalized.length !== ED25519_PRIVATE_SEED_BYTES) throw new Error("Ed25519 private seed must be 32 bytes");
  return Buffer.concat([ED25519_PKCS8_PREFIX, normalized]);
}

export function extractEd25519PrivateSeed(privateKeyPkcs8: Uint8Array): Buffer {
  const privateKey = createPrivateKey({ key: Buffer.from(privateKeyPkcs8), format: "der", type: "pkcs8" });
  const jwk = privateKey.export({ format: "jwk" });
  if (jwk.kty !== "OKP" || jwk.crv !== "Ed25519" || typeof jwk.d !== "string") throw new Error("expected Ed25519 private key");
  const seed = Buffer.from(jwk.d, "base64url");
  if (seed.length !== ED25519_PRIVATE_SEED_BYTES) throw new Error("unexpected Ed25519 private seed length");
  return seed;
}
...
export function signAtelierWalletAuthMessage(
  ...
  return Object.freeze({ address: normalizedAddress, timestamp: normalizedTimestamp, message, signatureBase58: base58Encode(signature) });
}
