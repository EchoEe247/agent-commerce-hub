...
function sanitized(value: unknown, depth = 0): unknown {
  ...
  if (typeof value === "string") return value.replace(/\s+/g, " ").slice(0, 1500);
  return value;
}
...
function rejectionSummary(job: JsonObject): string {
  ...
  if (value !== null && typeof value === "object") return JSON.stringify(sanitized(value)).slice(0, 1800);
  return "none";
}
...
async function main(): Promise<void> {
  ...
  const contract = readCodeTestContract(before, EXPECTED_FILENAME);
  ...
  console.log(`SUBMIT_RESPONSE_SANITIZED=${JSON.stringify(sanitized(submitResponse.body)).slice(0, 6000)}`);
  ...
  console.log("OUTPUT_DATA_SHAPE=files_map");
  console.log(`OUTPUT_FILE=${EXPECTED_FILENAME}`);
  ...
  for (let attempt = 0; attempt < 30; attempt += 1) {
    ...
    if (statusResponse.status === 200) {
      ...
      verificationResult = JSON.stringify(
        sanitized(after.verification_result ?? after.verificationResult ?? null),
      ).slice(0, 3000);
      const terminal = ["completed", "paid", "verified", "failed", "refunded", "open", "closed"].includes(afterStatus);
      if (terminal || lastRejection !== "none") break;
    }
    if (attempt < 29) await sleep(2_000);
  }

  const verification = submitResponse.body.verification;
  const syncPassed =
    verification !== null &&
  ...
  const syncFailed =
    ...
    typeof verification === "object" &&
    !Array.isArray(verification) &&
    (verification as JsonObject).passed === false;
  ...
  const oracleOutcome = syncPassed || paid
    ? "passed"
    : syncFailed || lastRejection !== "none" || ["failed", "refunded", "open"].includes(afterStatus)
      ? "failed"
      : "pending";
...
main().catch((error: unknown) => {
  ...
  console.error(`SUBMISSION_BROKER_HELPER_FAILED=${message.slice(0, 800)}`);
