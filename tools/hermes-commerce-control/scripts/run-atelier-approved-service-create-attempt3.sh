...
ATTEMPT3="${ROOT}/receipts/atelier-service-create-attempt-3.json"
STATE="${ROOT}/state/atelier-readme-service.json"

if [[ ! -f "${ATTEMPT1}" ]]; then
  echo "ERROR: attempt-1 receipt is required before attempt 3" >&2
  exit 2
fi
if [[ ! -f "${ATTEMPT2}" ]]; then
  echo "ERROR: attempt-2 receipt is required before attempt 3" >&2
  exit 2
fi
if [[ -e "${ATTEMPT3}" ]]; then
  echo "ERROR: attempt-3 receipt already exists; refusing retry" >&2
  exit 2
fi
if [[ -e "${STATE}" ]]; then
...
export ATELIER_SERVICE_RECEIPT_PATH="${ATTEMPT3}"
cleanup() {
