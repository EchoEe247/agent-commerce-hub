...
test("unpaid POST /v1/profile returns HTTP 402 with PAYMENT-REQUIRED response header", async () => {
  ...
  assert.ok(decoded.x402Version === 2 || decoded.accepts);
  ...
  assert.ok(!response.json().dataset);
  await app.close();
  await fac.close();
});
...
test("all six portfolio routes are x402 protected at exact route-specific prices with valid Bazaar metadata", async () => {
  ...
  try {
    for (const testCase of PORTFOLIO_PAYMENT_CASES) {
      const response = await app.inject({ method: "POST", url: testCase.path, payload: testCase.payload });
      ...
      assert.equal(validation.valid, true, `${testCase.path} Bazaar extension must validate: ${JSON.stringify(validation.errors)}`);
...
test("Bazaar extension passes official validateDiscoveryExtension with zero errors", async () => {
  ...
  const response = await app.inject({ method: "POST", url: "/v1/profile", payload: { format: "json", records: [{ id: 1 }] } });
  assert.equal(response.statusCode, 402);
  const bazaar = decodePaymentRequired(response).extensions.bazaar;
  assert.ok(bazaar);
