...
test("Product 11 unpaid request advertises exact $0.02 x402 payment and valid ticker-or-CIK Bazaar metadata", async () => {
  ...
  const app = buildApp({
    ...
    secCompanySnapshot: async () => ({ schema_version: "1.0" }),
  });
  ...
  try {
    ...
    assert.equal(quote.amount, "20000");

    const bazaar = decoded.extensions?.bazaar;
    assert.ok(bazaar, "Product 11 must advertise Bazaar discovery metadata");
    ...
    assert.ok(Array.isArray(bodySchema.oneOf));
    assert.deepEqual(bodySchema.oneOf.map((choice) => choice.required), [["ticker"], ["cik"]]);
    assert.equal(bazaar.info.output.example.schema_version, "1.0");
    assert.ok(bazaar.info.output.example.company);
    assert.ok(bazaar.info.output.example.filings);
    assert.ok(bazaar.info.output.example.facts);
  } finally {
    await app.close();
