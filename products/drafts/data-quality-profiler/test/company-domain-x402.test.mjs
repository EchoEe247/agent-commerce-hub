...
test("Product 10 unpaid request advertises exact $0.02 x402 payment and valid Bazaar metadata", async () => {
  ...
  const app = buildApp({
    ...
    companyDomainIntelligence: async () => ({ schema_version: "1.0" }),
  });
  ...
  try {
    ...
    assert.ok(bazaar, "Product 10 must advertise Bazaar discovery metadata");
    ...
    assert.deepEqual(bodySchema.required, ["domain"]);
    assert.equal(bodySchema.properties.domain.type, "string");
    assert.equal(bazaar.info.output.example.schema_version, "1.0");
    assert.ok(bazaar.info.output.example.dns);
    assert.ok(bazaar.info.output.example.domain);
    assert.ok(bazaar.info.output.example.website);
  } finally {
    await app.close();
